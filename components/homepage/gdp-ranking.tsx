      ...
      {rows.map((row, index) => {
        ...
        return (
          <li key={row.countryCode} className="grid grid-cols-[2rem_1fr_auto] items-center gap-3 py-4 sm:grid-cols-[2.5rem_10rem_1fr_9rem] sm:gap-5">
            <span className={`text-center font-mono text-sm ${index < 3 ? "font-semibold text-sky-700" : "text-slate-400"}`}>{index + 1}</span>
            ...
            <span className="hidden h-2 overflow-hidden rounded-full bg-slate-100 sm:block">
              <span className="block h-full rounded-full bg-gradient-to-r from-blue-600 to-sky-300" style={{ width: `${width}%` }} />
            </span>
