...
if (!databaseConfigured) warnings.push("DATABASE_URL is not configured; public pages will continue using the verified static snapshot.");
if (requireDatabase && !databaseConfigured) failures.push("DATABASE_URL is required for production database sync.");
